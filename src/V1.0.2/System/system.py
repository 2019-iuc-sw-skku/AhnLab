...
###############파일로 정보 저장######################
f1 = open("./system_orgout.txt", 'w')  # 원본 정보## #폴더명 변경함
f2 = open("./system_out.txt", 'w')  # 간략한 정보##  #폴더명 변경함
##############환경변수 수집########################
...
def execute_openssl(certslist):
    command1 = "System/exessl.sh"
    certdata = []
    result = subprocess.Popen(
        command1, stdout=subprocess.PIPE, shell=True).stdout
    certdata = result.read().strip().decode().split()
    result.close()
    for f in certdata:
        check_cmd = "ls -al /etc/ssl/certs/ | grep " + f
        if(os.popen(check_cmd).read()[0] == 'd'):
            continue
        command2 = "openssl x509 -noout -in "+"/etc/ssl/certs/"+f+" -text"
        result2 = subprocess.Popen(
            command2, stdout=subprocess.PIPE, shell=True).stdout
...
def get_sysstat():
    ...
    # print("Name".ljust(20)+"Expire Date".ljust(20)+"Issuer".ljust(20) +
    #      "Subject".ljust(20)+"Serial Number".ljust(20)+"Location".ljust(20))
    f1.write("인증서 정보\n")
    f2.write("인증서 정보\n")
    f1.write("Name".ljust(20)+"Expire Date".ljust(20)+"Issuer".ljust(20) +
             "Subject".ljust(20)+"Serial Number".ljust(20)+"Location".ljust(20))
    f2.write("Name".ljust(20)+"Expire Date".ljust(20)+"Issuer".ljust(20) +
             "Subject".ljust(20)+"Serial Number".ljust(20)+"Location".ljust(20))
